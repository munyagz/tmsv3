...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.send-floats.update", [$sendFloat->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="date_sent">{{ trans('cruds.sendFloat.fields.date_sent') }}</label>
                <input class="form-control date {{ $errors->has('date_sent') ? 'is-invalid' : '' }}" type="text" name="date_sent" id="date_sent" value="{{ old('date_sent', $sendFloat->date_sent) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $sendFloat->amount) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('transaction_ref') ? 'is-invalid' : '' }}" type="text" name="transaction_ref" id="transaction_ref" value="{{ old('transaction_ref', $sendFloat->transaction_ref) }}" required>
