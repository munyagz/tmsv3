...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.money-receiveds.update", [$moneyReceived->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="date_received">{{ trans('cruds.moneyReceived.fields.date_received') }}</label>
                <input class="form-control date {{ $errors->has('date_received') ? 'is-invalid' : '' }}" type="text" name="date_received" id="date_received" value="{{ old('date_received', $moneyReceived->date_received) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $moneyReceived->amount) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('transaction_ref') ? 'is-invalid' : '' }}" type="text" name="transaction_ref" id="transaction_ref" value="{{ old('transaction_ref', $moneyReceived->transaction_ref) }}">
